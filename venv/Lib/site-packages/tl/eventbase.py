...
from tl.floodcontrol import floodcontrol
...
class EventBase(LazyDict):
    ...
    def display(self):
        ...
        for res in result: self.reply(self.replytxt, result)
        ...
        if not self.user and target and not self.nodispatch:
            ...
            if not u: 
                ...
                if cfg.auto_register and self.iscommand:
                    ...
                    if u: logging.warn("applied auto_register")
                    else: logging.error("can't add %s to users database" % target)
            if u:
                msg = "%s event on %s (%s)" % (self.cbtype, self.bot.cfg.name, u.data.name)
