...
from .utils.generic import splittxt, toenc, fromenc, waitforqueue, strippedtxt, waitevents, stripcolor
from .utils.locking import lock_object, release_object
from .utils.url import decode_html_entities
...
from .utils.resolve import resolve_host, resolve_ip
...
from .id import get_bid, get_uid
...
class BotBase(LazyDict):
    ...
    def start(self, connect=True, join=True):
        ...
        if self.started: logging.info("%s - already started" % self.cfg.name) ; return
        fleet = getfleet()
        fleet.addbot(self)
        self.stopped = False
    ...
    def say(self, channel, txt, result=[], how="msg", event=None, nr=375, extend=0, dot=", ", showall=False, *args, **kwargs):
        ...
        logging.info("saying to %s" % channel)
        if event:
    ...
    def saynocb(self, channel, txt, result=[], how="msg", event=None, nr=375, extend=0, dot=", ", showall=False, *args, **kwargs):
        logging.info("saying to %s (without callbacks)" % channel)
    ...
    def makeresponse(self, txt, result=[], dot=", ", nosort=False, *args, **kwargs):
        ...
        if dot == "count": dot = "<br>"
        if txt:
             if res and self.type in ["console", ]: ret = "%s<br><br>" % txt + dot.join(res)
